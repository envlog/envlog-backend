	...
	Data: [
		{
			...
			X_Max_Value: number;
			X_Max_Freq: number;
			X_Max_FreqUnit: string;
		},
		{
			Y_Status: string;
			Y_Max_Value: number;
			Y_Max_Freq: number;
			Y_Max_FreqUnit: string;
		},
		{
			Z_Status: string;
			Z_Max_Value: number;
			Z_Max_Freq: number;
			Z_Max_FreqUnit: string;
		}
	];
}
...
export type SensorInterface = Omit<
	Required<SensorSchema>,
	'Data' | 'Received'
> & { Name: string; Enabled: boolean; Group: string | null };

export type SocketObject = Omit<SensorSchema, 'Received' | 'Data'> & {
	Value?: string;
	Unit?: string;
	Voltage?: number;
};
