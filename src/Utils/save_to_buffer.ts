...
export const saveToBuffer = async (mqttObject: MQTTPayload) => {
	...
	} else finalObject.Data = JSON.stringify(minifiedObject.Data);
	...
	if (sensors[Type].counter === Number(process.env.ELEMENTS_PER_BUFFER)) {
		try {
			...
			console.log(
				`[DATABASE] Saved ${process.env.ELEMENTS_PER_BUFFER} ${Type} elements to database!`
			);
		} catch (err: any) {
			console.log(
				`[DATABASE] Error trying to save ${process.env.ELEMENTS_PER_BUFFER} ${Type} elements to database: ${err}.`
			);
		}
	}
};
