...
registerRouter.post('/', auth_1.requiresNoAuth, (0, express_validator_1.body)('username').isLength({ min: Number(process.env.MIN_USERNAME_LEN) }).trim().escape().withMessage("Il nome utente deve contenere almeno ".concat(process.env.MIN_USERNAME_LEN, " caratteri!")), (0, express_validator_1.body)('email').isEmail().normalizeEmail().withMessage("L'email non è valida!"), (0, express_validator_1.body)('password').isLength({ min: Number(process.env.MIN_PASS_LEN) }).trim().escape().withMessage("La password deve contenere almeno ".concat(process.env.MIN_PASS_LEN, " caratteri!")), (0, express_validator_1.body)('passwordConfirmation').custom(auth_1.passwordsMatch), function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
